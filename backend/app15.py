...
# PROYECTOS
@app.route("/proyectos2", methods=["GET"])
@session_required
def get_proyectos2(current_user_id):
    conn = None
    try:
        conn = get_db_connection()
        if not conn:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("""
                SELECT
                    ...
                    ua.user_id  AS actualizado_por_id,
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("""
                SELECT
                    ...
                    ), '[]'::json) AS proyectos_observaciones
...
def create_proyecto(current_user_id):
    ...
    try:
        ...
        forbidden = {"user_id", "actualizado_por", "fecha_actualizacion"}
...
def delete_proyecto(current_user_id, pid):
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "DELETE FROM proyectos WHERE id = %s RETURNING id",
                (pid,)
            )
            deleted = cur.fetchone()
